...
class GraphAE(nn.Module):
    ...
    def init_layers(self, batch):
        ...
        for l in range(self.layer_num):
            ...
            neighbor_mask_lst = pc_mask[
                neighbor_id_lstlst].contiguous()  # out_pn*max_neighbor_num neighbor is 1 otherwise 0
            ...
            if (residual_rate > 0):
                ...
                if (out_point_num != in_point_num):
                    p_neighbors = nn.Parameter(
                        (torch.randn(out_point_num, max_neighbor_num) / (avg_neighbor_num)).cuda())
            ...
            layer = (
                in_channel, out_channel, in_point_num, out_point_num, weight_num, max_neighbor_num, neighbor_num_lst,
                neighbor_id_lstlst, conv_layer, residual_layer, residual_rate, neighbor_mask_lst,
                zeros_batch_outpn_outchannel)

            self.layer_lst += [layer]

            # print ("in_channel", in_channel, "out_channel", out_channel, "in_point_num", in_point_num, "out_point_num",
            #        out_point_num, "weight_num", weight_num,
            #        "max_neighbor_num", max_neighbor_num, "avg_neighbor_num", avg_neighbor_num,
            #        "effective_w_weights_rate", effective_w_weights_rate)
    ...
    def init_test_mode(self):
        for l in range(len(self.layer_lst)):
            ...
            if (len(conv_layer) != 0):
                ...
                w_weights = ""
                w_weights = raw_w_weights * neighbor_mask_lst.view(out_pn, max_neighbor_num, 1).repeat(1, 1,
                                                                                                       weight_num)  # out_pn*max_neighbor_num*weight_num
                weights = torch.einsum('pmw,wc->pmc',
                                       [w_weights, weights])  # out_pn*max_neighbor_num*(out_channel*in_channel)
            ...
            self.layer_lst[
                l] = in_channel, out_channel, in_pn, out_pn, weight_num, max_neighbor_num, neighbor_num_lst, neighbor_id_lstlst, conv_layer, residual_layer, residual_rate, neighbor_mask_lst, zeros_batch_outpn_outchannel
    ...
    def forward_one_conv_layer_batch_during_test(self, in_pc, layer_info, is_final_layer=False):  #in_pc: torch.Size([1, 873, 3])
        ...
        if (len(conv_layer) != 0):
            (weights,
             bias) = conv_layer  # weight_num*(out_channel*in_channel)   out_point_num* max_neighbor_num* weight_num
